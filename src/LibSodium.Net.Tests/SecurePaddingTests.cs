...
namespace LibSodium.Tests
{
	public class SecurePaddingTests
	{
		...
		public void Pad_ShortData_PadsCorrectly()
		{
			...
			var padded = SecurePadding.Pad(buffer, unpaddedLen: 3, blockLen: 8).ToArray();
		...
		public void Pad_ExactBlockSize_PadsCorrectly()
		{
			...
			var padded = SecurePadding.Pad(buffer, unpaddedLen: 8, blockLen: 8).ToArray();
			byte[] expected = { 
				0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 
		...
		public void Pad_EmptyData_PadsCorrectly()
		{
			...
			var padded = SecurePadding.Pad(buffer, unpaddedLen: 0, blockLen: 8).ToArray();
		...
		public void Unpad_PaddedData_UnpadsCorrectly()
		{
			...
			var unpadded = SecurePadding.Unpad(padded, blockLen: 8).ToArray();
		...
		public void Unpad_OnlyPadding_UnpadsCorrectly()
		{
			...
			var unpaddedLen = SecurePadding.Unpad(padded, blockLen: 8).Length;
		...
		public void Unpad_InvalidPadding_ThrowsSodiumException()
		{
			...
			AssertLite.Throws<LibSodiumException>(() => SecurePadding.Unpad(invalidPadded1, blockLen: 8));
			AssertLite.Throws<LibSodiumException>(() => SecurePadding.Unpad(invalidPadded2, blockLen: 8));
		}
		...
		public void Pad_ZeroBlockSize_ThrowsArgumentException()
		{
			...
			AssertLite.Throws<ArgumentException>(() => SecurePadding.Pad(buffer, unpaddedLen: 3, blockLen: 0));
		}
